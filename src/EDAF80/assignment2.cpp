...
edaf80::Assignment2::run()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (interpolate) {
			glm::vec3 new_pos;
			int p_1 = ((int) elapsed_time_s) % control_point_locations.size();
			int p_2 = (p_1 + 1) % control_point_locations.size();
			double integral_part;
			float delta_p = modf(elapsed_time_s, &integral_part);

			if (use_linear) {
				new_pos = interpolation::evalLERP(control_point_locations[p_1], 
												control_point_locations[p_2], 
												delta_p);
			}
			else {
				int p_0 = (p_1 - 1 + control_point_locations.size()) % control_point_locations.size();
				int p_3 = (p_2 + 1) % control_point_locations.size();
				new_pos = interpolation::evalCatmullRom(control_point_locations[p_0],
														control_point_locations[p_1],
														control_point_locations[p_2],
														control_point_locations[p_3],
														catmull_rom_tension,
														delta_p);
			}

			circle_rings.get_transform().SetTranslate(new_pos);
		}
